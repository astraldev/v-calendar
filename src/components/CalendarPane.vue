<template>
  <div ref="pane" class="c-pane" :style="paneStyle">
    ...
    <slot name="header" v-bind="page_">
      <div class="c-header" :style="headerStyle">
        ...
        <div class="c-arrow-layout">
          <slot v-if="!hideLeftButton" name="header-left-button" v-bind="page_">
            <svg-icon
              name="leftArrow"
        ...
        <div :class="['c-title-layout', titleClass]">
          ...
          <popover
            ...
            :force-hidden="navForceHidden"
            toggle-visible-on-click
            is-interactive
          >
            <!--Title content-->
            <transition-group tag="div" class="c-title-anchor" :name="titleTransition_">
              <div
                v-for="p in pages.filter((p) => p === page_)"
                :key="p.key"
                class="c-title"
                :style="titleStyle"
              >
                <slot name="header-title" v-bind="p">
                  {{ p.title }}
                </slot>
              </div>
            </transition-group>
            ...
            <template #popover-content>
              ...
              >
                ...
                <template v-for="slot in navSlots" #[slot]="props">
        ...
        <!--Header next button-->
        <div class="c-arrow-layout">
          <slot v-if="!hideRightButton" name="header-right-button" v-bind="page_">
            <svg-icon
              name="rightArrow"
    ...
    <div class="c-weekdays" :style="weekdaysStyle_">
      <div v-for="(weekday, i) in weekdayLabels" :key="i + 1" class="c-weekday">
        {{ weekday }}
      </div>
    </div>
    ...
    />
    <!--Weeks-->
    <div class="c-weeks" :style="weeksStyle_">
      <transition-group
        tag="div"
...
import { evalFn, getWeekdayDates } from "@/utils/helpers";
...
export default {
  ...
  computed: {
    navSlots() {
      return ["nav-left-button", "nav-right-button"].filter((slot) => this.$slots[slot]);
    },
    weekdayLabels() {
      return getWeekdayDates({ firstDayOfWeek: defaults.firstDayOfWeek }).map((d) =>
        format(d, this.formats.weekdays)
      );
    },
    titleClass() {
  ...
  methods: {
    ...
    move(pageInfo) {
      ...
      } else if (this.minPage && pageIsBeforePage(todayComps, this.minPage)) {
        this.forceMove(this.minPage);
      } else if (this.maxPage && pageIsAfterPage(pageInfo, this.maxPage)) {
    ...
    forceMove(pageInfo) {
      ...
      if (
        !pageInfo ||
        (pageInfo.month === this.page_.month && pageInfo.year === this.page_.year)
      )
        return;
      // Set the active page
